...
export default function CourseSelectionSection({
  ...
  const selectBase =
    "w-full bg-slate-50 border-2 border-slate-100 rounded-xl sm:rounded-2xl px-4 sm:px-6 md:px-8 py-3 sm:py-4 md:py-5 text-base sm:text-lg md:text-xl text-slate-900 transition-all focus:bg-white focus:border-cu-blue appearance-none cursor-pointer font-medium";
  const label =
    "block text-xs sm:text-sm font-black text-slate-500 uppercase tracking-widest mb-3 sm:mb-4 md:mb-5 px-1 sm:px-2";
    ...
    <div className="animate-fade-in [animation-delay:200ms]">
      ...
      <div className="relative group">
        ...
        <div className="relative">
          <select
            ...
            value={
              courses.data.courses.find((c) => c.title === selectedCourse)
                ?.id || ""
            }
            onChange={(e) => {
          ...
          >
            <option value="" disabled>
              --- اختر البرنامج التدريبي (اختياري) ---
            </option>
          ...
          <div className="absolute left-3 sm:left-5 md:left-8 top-1/2 -translate-y-1/2 pointer-events-none">
            <svg
              className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-slate-400 group-hover:text-cu-blue transition-colors"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      <div className="mt-5 sm:mt-7 md:mt-10 p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-[2rem] bg-cu-blue/5 border border-cu-blue/10 flex items-start sm:items-center gap-4 sm:gap-6">
        ...
        <p className="text-sm sm:text-base md:text-lg text-slate-700 font-bold arabic-text-premium leading-relaxed">
          جميع البرامج المقدمة معتمدة وتخضع لمعايير جودة عالية لضمان تقديم محتوى
          تدريبي احترافي.
        </p>
      </div>
    </div>
