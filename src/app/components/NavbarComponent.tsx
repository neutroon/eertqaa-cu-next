...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-100 transition-all duration-700 ease-in-out ${scrolled ? "pt-4" : "pt-0"
        }`}
    >
      <div className="max-w-7xl mx-auto px-3">
        <div
          className={`flex items-center justify-between transition-all duration-700 ease-in-out ${scrolled
        ...
        >
          ...
          <div className="flex items-center gap-8 group">
            <div className="relative group/logo">
              ...
              <div
                className={`absolute inset-0 -inset-y-2 -inset-x-2 rounded-b-full transition-all duration-700 ${scrolled
              ...
              <Image
                ...
                className={`object-contain transition-all duration-700 relative z-10 drop-shadow-2xl ${!scrolled ? "brightness-[1.05] saturate-[1.1]" : ""
                  }`}
              />
            </div>

            <div className={`hidden sm:flex flex-col border-l-2 pl-8 transition-all duration-700 ${scrolled ? "border-slate-200" : "border-white/20"
              }`}>
              <span className={`text-2xl font-black font-family-outfit tracking-tighter transition-colors duration-700 ${scrolled ? "text-slate-950" : "text-white"
                }`}>
                برامج جامعة القاهرة
              </span>
              <span className={`text-[10px] font-black tracking-[0.4em] uppercase transition-colors duration-700 ${scrolled ? "text-cu-blue" : "text-white/60"
                }`}>
                Cairo University
              </span>
            </div>
          ...
          <div className="flex items-center gap-8">
            <a
              href="#register"
              className={`btn-boutique py-4 px-10 text-base tracking-tight transition-all duration-700 ${scrolled
